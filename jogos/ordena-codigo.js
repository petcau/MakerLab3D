...
let dragSrc = null, dragFromZone = null;
...
let zonaLinhas  = []; // linhas na dropzone (ordem atual do aluno)
let blocosLivres = []; // linhas nos blocos disponíveis
...
async function init() {
  ...
  onAuthStateChanged(auth, async user => {
    ...
    try {
      ...
      document.title = 'Ordene o Código — ' + (cardData.nome || cardId);

      if (!cardData.ordena_desafios || cardData.ordena_desafios.length === 0) {
        erroLoad('Este card não tem o jogo Ordene o Código cadastrado.'); return;
      }

      tentativasPermitidas = cardData.ordena_tentativas || 3;
      resultadoDocId = user.uid + '_ordena_' + cardId;

      const resultSnap = await getDoc(doc(db, 'resultados_ordena', resultadoDocId));
      ...
      desafios = [...cardData.ordena_desafios];
      iniciarJogo();
...
function iniciarJogo() {
  ...
  document.getElementById('ord-box').style.display      = '';
...
function renderDesafio() {
  ...
  document.getElementById('ord-num').textContent      = 'Desafio ' + (atual + 1);
  document.getElementById('ord-pergunta').textContent = d.descricao || 'Ordene o código corretamente!';
  document.getElementById('ord-pts-badge').textContent = '+' + (pts % 1 === 0 ? pts : pts.toFixed(1)) + ' pt' + (pts !== 1 ? 's' : '');

  // Separar fixas e móveis
  const linhas = (d.codigo || '').split('\n');
  const fixas  = d.linhas_fixas || [];

  // Linhas móveis embaralhadas
  const moveis = linhas
    .map((txt, i) => ({ idx: i, txt, fixo: fixas.includes(i) }))
    .filter(l => !l.fixo)
    .sort(() => Math.random() - 0.5);

  // Montar estado inicial: zona com fixas, blocos com móveis
  zonaLinhas   = linhas.map((txt, i) => fixas.includes(i) ? { idx: i, txt, fixo: true } : null);
  blocosLivres = moveis;

  renderZona(false);
  renderBlocos();

  document.getElementById('ord-feedback').style.display = 'none';
  document.getElementById('btn-verificar').style.display = '';
  document.getElementById('btn-prox').style.display     = 'none';
}

function renderZona(revelar) {
  const zone = document.getElementById('ord-dropzone');
  const d    = desafios[atual];
  const linhasCorr = (d.codigo || '').split('\n');

  zone.innerHTML = '';
  zonaLinhas.forEach((item, pos) => {
    if (item === null) {
      // Slot vazio
      const slot = document.createElement('div');
      slot.className   = 'drop-placeholder';
      slot.dataset.pos = pos;
      slot.textContent = 'Solte aqui...';
      setupDropTarget(slot, 'zona', pos);
      zone.appendChild(slot);
    } else {
      const bloco = criarBloco(item, pos, 'zona', revelar, linhasCorr);
...
function renderBlocos() {
  const wrap = document.getElementById('ord-blocos');
  wrap.innerHTML = '';
  if (blocosLivres.length === 0) {
    wrap.innerHTML = '<div style="color:#bbb;font-size:13px;text-align:center;padding:20px;">Todos os blocos foram colocados!</div>';
    return;
  }
  blocosLivres.forEach((item, i) => {
    const bloco = criarBloco(item, i, 'blocos', false, null);
    wrap.appendChild(bloco);
  });
}

function criarBloco(item, pos, zona, revelar, linhasCorr) {
  const div = document.createElement('div');
  div.className = 'cod-bloco' + (item.fixo ? ' fixo' : '');
  div.dataset.idx  = item.idx;
  div.dataset.zona = zona;
  div.dataset.pos  = pos;

  if (revelar && zona === 'zona' && !item.fixo) {
    const correto = linhasCorr && zonaLinhas[pos] && zonaLinhas[pos].idx === pos;
    div.classList.add(correto ? 'correto' : 'incorreto');
  }

  const icon = revelar && zona === 'zona' && !item.fixo
    ? (linhasCorr && zonaLinhas[pos] && zonaLinhas[pos].idx === pos ? '✅' : '❌')
    : '';

  div.innerHTML =
    '<span class="cod-bloco-handle">' + (item.fixo ? '🔒' : '⠿') + '</span>' +
    '<span class="cod-bloco-code">' + item.txt.replace(/</g, '&lt;') + '</span>' +
    (item.fixo ? '<span class="cod-bloco-fixo-tag">fixo</span>' : '') +
    (icon ? '<span class="cod-bloco-icon">' + icon + '</span>' : '');

  if (!item.fixo) {
    div.draggable = true;
    setupDrag(div, zona, pos);
  }
  return div;
}

// ── Drag & Drop ──────────────────────────────────
function setupDrag(el, zona, pos) {
...
function setupDropTarget(el, zona, pos) {
  ...
  el.addEventListener('drop', e => {
    ...
    if (!dragSrc) return;
    moverBloco(dragSrc, { zona, pos });
    dragSrc = null;
  });
}

// Permitir drop no container geral dos blocos
document.addEventListener('DOMContentLoaded', () => {
  const blocosWrap = document.getElementById('ord-blocos');
  if (blocosWrap) {
    blocosWrap.addEventListener('dragover', e => { e.preventDefault(); blocosWrap.classList.add('drag-over'); });
    blocosWrap.addEventListener('dragleave', () => blocosWrap.classList.remove('drag-over'));
    blocosWrap.addEventListener('drop', e => {
      e.preventDefault();
      blocosWrap.classList.remove('drag-over');
      if (!dragSrc || dragSrc.zona === 'blocos') return;
      // Mover de volta para blocos
      const item = zonaLinhas[dragSrc.pos];
      if (!item || item.fixo) return;
      zonaLinhas[dragSrc.pos] = null;
      blocosLivres.push(item);
      renderZona(false);
      renderBlocos();
      dragSrc = null;
    });
...
function moverBloco(src, dest) {
  let itemSrc = null;

  if (src.zona === 'zona') {
    itemSrc = zonaLinhas[src.pos];
    if (!itemSrc || itemSrc.fixo) return;
    zonaLinhas[src.pos] = null;
  } else {
    itemSrc = blocosLivres[src.pos];
    blocosLivres.splice(src.pos, 1);
  }

  if (dest.zona === 'zona') {
    const atual = zonaLinhas[dest.pos];
    if (atual && !atual.fixo) {
      // Trocar — devolver o que estava no slot para blocos
      blocosLivres.push(atual);
    } else if (atual && atual.fixo) {
      // Slot ocupado por fixo — cancelar
      if (src.zona === 'blocos') blocosLivres.splice(src.pos, 0, itemSrc);
      else zonaLinhas[src.pos] = itemSrc;
      return;
    }
    zonaLinhas[dest.pos] = itemSrc;
  } else {
    blocosLivres.push(itemSrc);
  }
...
window.verificar = function() {
  ...
  const vazios = zonaLinhas.filter(l => l === null).length;
  if (vazios > 0 || blocosLivres.length > 0) {
    const fb = document.getElementById('ord-feedback');
    fb.className = 'ord-feedback erro';
    fb.textContent = '⚠️ Coloque todos os blocos na área de montagem antes de verificar!';
  ...
  const pts      = parseFloat(d.pontos) || 1.0;
  const fixas    = d.linhas_fixas || [];
  const moveis   = linhas.map((_, i) => i).filter(i => !fixas.includes(i));

  // Verificar cada posição móvel
  let todoCerto = true;
  zonaLinhas.forEach((item, pos) => {
    if (!item || item.fixo) return;
    if (item.idx !== pos) todoCerto = false;
  });
  ...
  if (todoCerto) {
    ...
    pontosGanhos += pts;
    somAcerto();
    fb.className = 'ord-feedback acerto';
    fb.textContent = '✅ Perfeito! ' + (d.feedback || 'O código está na ordem correta!');
  } else {
    erros++;
    somErro();
    fb.className = 'ord-feedback erro';
    fb.textContent = '❌ ' + (d.feedback || 'Algumas linhas estão fora de ordem. Veja os destaques!');
  }
  fb.style.display = 'block';
...
function mostrarFinal() {
  ...
  if (pct === 100)    { emoji='🏆'; titulo='Código Ordenado!'; msg='Perfeito! Você ordenou todos os códigos corretamente!'; }
  else if (pct >= 70) { emoji='⬇️'; titulo='Muito bem!'; msg='Você acertou ' + acertos + ' de ' + total + '. Continue praticando!'; }
  else if (pct >= 40) { emoji='💡'; titulo='Bom esforço!'; msg='Você acertou ' + acertos + ' de ' + total + '. Revise a lógica do código!'; }
  else                { emoji='🔄'; titulo='Não desista!'; msg='A lógica de programação leva tempo. Tente de novo!'; }
...
async function registrarTentativa() {
  ...
  inicioJogo = Date.now();
  try {
    const ref  = doc(db, 'resultados_ordena', resultadoDocId);
    ...
    tentativasUsadas++;
  } catch(e) { console.warn('Tentativa ordena:', e); }
}
...
async function salvarResultado(acertos, pontos, total, concluido) {
  ...
  const queries = ['resultados_quiz','resultados_bug','resultados_comp','resultados_ordena','resultados_complete','resultados_conecta','resultados_box','resultados_binario']
...
function mostrarBloqueado(dados) {
  document.getElementById('loading').style.display = 'none';
  document.getElementById('ord-box').innerHTML = `
    <div style="padding:40px 28px;text-align:center;">
      ...
      ${dados ? `<div style="background:#fef3e2;border:1.5px solid #e67e22;border-radius:16px;padding:16px;margin-bottom:24px;display:inline-block;">
        <div style="font-family:'Nunito',sans-serif;font-size:24px;font-weight:900;color:#7d3500;">${dados.melhor_acertos}/${dados.total_desafios} corretos</div>
        <div style="color:#d35400;font-weight:700;">${dados.melhor_pontos} pontos</div>
